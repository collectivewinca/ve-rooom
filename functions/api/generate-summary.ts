import { saveCachedResult, getMeetingPrompt, getUserPrompt, getMeetingMeta, getParticipants, addSummaryVersion, isEmailSent, markEmailSent, getSummaryLockOwner, getCachedResult } from "../lib/kv";
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, waitUntil }) => {
	...
	// Prevent duplicate summaries: if the webhook already generated a summary
	// (or is actively generating one), defer to it instead of producing a V2.
	if (body.meetingId) {
		const cached = await getCachedResult(env.MEETING_CACHE, body.meetingId, body.sessionId);
		if (cached?.summary) {
			console.log("[generate-summary] Summary already cached for", body.meetingId, "session", body.sessionId || "(none)", "— returning it");
			return jsonResponse(200, { status: "ok", summary: cached.summary });
		}
		const summaryLockOwner = await getSummaryLockOwner(env.MEETING_CACHE, body.meetingId, body.sessionId);
		if (summaryLockOwner) {
			console.log("[generate-summary] Summary lock held by", summaryLockOwner, "— deferring, returning processing");
			return jsonResponse(200, {
				status: "processing",
				message: "Summary is being generated by the server. Please wait.",
				chunksDone: 0,
				totalChunks: 0,
			});
		}
